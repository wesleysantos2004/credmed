// Sistema de gerenciamento de estado - Preparado para integração com backend
// TODO: Substituir por chamadas para API REST

const AppStorage = {
  // Cache local para dados durante a sessão (substitui localStorage)
  _cache: {
    solicitacoes: [],
    nextId: 1,
  },

  // Inicializar dados de exemplo (temporário, será substituído pela API)
  init() {
    this.resetData();
  },

  // Reset para dados iniciais de exemplo (temporário)
  resetData() {
    const solicitacoes = [
      {
        id: "SOL-2025-00042",
    ...
    // Armazenar em cache local (substitui localStorage)
    this._cache.solicitacoes = solicitacoes;
    this._cache.nextId = 48;
  },

  // Obter todas as solicitações (será substituído por GET /api/v1/requests)
  getSolicitacoes() {
    // TODO: return fetch('/api/v1/requests').then(r => r.json())
    return this._cache.solicitacoes;
  },

  // Obter solicitação por ID (será substituído por GET /api/v1/requests/:id)
  getSolicitacao(id) {
    // TODO: return fetch(`/api/v1/requests/${id}`).then(r => r.json())
    const solicitacoes = this.getSolicitacoes();
    return solicitacoes.find((s) => s.id === id || s.numero === id);
  },

  // Obter solicitações de um médico específico (será substituído por GET /api/v1/requests?doctor_id=X)
  getSolicitacoesMedico(medicoId) {
    // TODO: return fetch(`/api/v1/requests?doctor_id=${medicoId}`).then(r => r.json())
    const solicitacoes = this.getSolicitacoes();
    return solicitacoes.filter((s) => s.medicoId === medicoId);
  },

  // Salvar solicitações (será removido quando integrar com backend)
  saveSolicitacoes(solicitacoes) {
    this._cache.solicitacoes = solicitacoes;
  },

  // Criar nova solicitação (será substituído por POST /api/v1/requests)
  criarSolicitacao(dados) {
    // TODO: return fetch('/api/v1/requests', { method: 'POST', body: JSON.stringify(dados) })
    const solicitacoes = this.getSolicitacoes();
    const nextId = this._cache.nextId;
  ...
  // Função para limpar todos os dados (localStorage + cache)
  clearAllData() {
    // Limpar localStorage
    ...
    console.log("✅ Todos os dados foram limpos (localStorage + cache)");
  },
};
...
// Função global para limpeza rápida
